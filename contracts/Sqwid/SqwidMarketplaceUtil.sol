...
contract SqwidMarketplaceUtil is Ownable {
    ...
    modifier pagination(uint256 pageNumber, uint256 pageSize) {
        ...
        require(
            pageSize <= 100 && pageSize > 0,
            "SqwidMarketUtil: Invalid page size"
        );
        _;
    }
    ...
    modifier idsSize(uint256 size) {
        require(
            size <= 100 && size > 0,
            "SqwidMarketUtil: Invalid number of ids"
        );
        _;
    }
     ...
     */
    function setMarketContractAddress(
        ISqwidMarketplace marketplace_
    ) external onlyOwner {
        marketplace = marketplace_;
    }
    ...
    function fetchItem(
        uint256 itemId
    ) public view returns (ItemResponse memory) {
     ...
     */
    function fetchAddressNumberItemsCreated(
    ...
    function fetchPosition(
        ...
        ISqwidMarketplace.Position memory position = marketplace.fetchPosition(
            positionId
        );
        ...
        ISqwidMarketplace.Item memory item = marketplace.fetchItem(
            position.itemId
        );
    ...
    function fetchPositions(
        ...
        uint256 count;
        for (uint256 i = startIndex; i > 0; i--) {
            ISqwidMarketplace.Position memory position = marketplace
                .fetchPosition(i);
            if (
     ...
     */
    function fetchAddressNumberPositions(
        address targetAddress
    ...
    function fetchAuctionBids(
        ...
        uint256 totalAddresses = marketplace
            .fetchAuctionData(positionId)
            .totalAddresses;
        ...
        for (uint256 i; i < totalAddresses; i++) {
            (address addr, uint256 amount) = marketplace.fetchBid(
                positionId,
                i
            );
            addresses[i] = addr;
        ...
        returns (AuctionBidded[] memory bids, uint256 totalPages)
    {
        if (newestToOldest) {
            return
                _fetchAddressBidsReverse(targetAddress, pageSize, pageNumber);
        } else {
     ...
     */
    function fetchAddressNumberBids(
        address targetAddress
    ) external view returns (uint256) {
        uint256 totalPositionCount = marketplace.currentPositionId();
        ...
        for (uint256 i; i < totalPositionCount; i++) {
            if (
                marketplace.fetchPosition(i + 1).state ==
    ...
    function fetchRaffleEntries(
        ...
        // Fill arrays
        for (uint256 i; i < totalAddresses; i++) {
            (address addr, uint256 amount) = marketplace.fetchRaffleEntry(
                positionId,
                i
            );
            addresses[i] = addr;
     ...
     */
    function fetchAddressNumberRaffles(
        address targetAddress
    ) external view returns (uint256) {
        uint256 totalPositionCount = marketplace.currentPositionId();
        ...
        returns (PositionResponse[] memory loans, uint256 totalPages)
    {
        if (newestToOldest) {
            return
                _fetchAddressLoansReverse(targetAddress, pageSize, pageNumber);
        } else {
     ...
     */
    function fetchAddressNumberLoans(
        address targetAddress
    ) external view returns (uint256) {
        uint256 totalPositionCount = marketplace.currentPositionId();
    ...
    function _fetchPositionsByItemId(
        uint256 itemId
    ) private view returns (ISqwidMarketplace.Position[] memory) {
        // Initialize array
        ISqwidMarketplace.Position[]
            memory items = new ISqwidMarketplace.Position[](
    ...
    function _fetchAddressBids(
        ...
        for (uint256 i = firstMatch; count < endIndex - startIndex + 1; i++) {
            if (
                marketplace.fetchPosition(i).state ==
                ISqwidMarketplace.PositionState.Auction
            ) {
                (
        ...
        for (uint256 i = firstMatch; count < endIndex - startIndex + 1; i++) {
            if (
                ...
                (
                    address[] memory addresses,
                    uint256[] memory amounts
                ) = fetchRaffleEntries(i);
                for (uint256 j; j < addresses.length; j++) {
                    if (addresses[j] == targetAddress) {
                        raffles[count] = RaffleEntered(
                            fetchPosition(i),
                            amounts[j]
                        );
                        count++;
     ...
     */
    function _checkExistsBytes(
        uint256 _id,
        bytes memory _packedBooleans
