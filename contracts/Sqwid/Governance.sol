...
contract SqwidGovernance {
    ...
    mapping(uint256 => mapping(address => bool))
        public minConfirmationsChangeApproved;
    ...
    modifier activeTx(uint256 _txId) {
        ...
        require(
            _txId > 0 && transaction.id == _txId,
            "Governance: Tx does not exist"
        );
        ...
        require(
            ...
            "Governance: Tx deadline has passed"
        );
        _;
    }
    ...
    modifier activeOwnersChange(uint256 _ownersChangeId) {
        ...
        require(
            !ownersChange.executed,
            "Governance: Owners change already executed"
        );
    ...
    modifier activeMinConfirmationsChange(uint256 _minConfirmationsChangeId) {
        MinConfirmationsChange
            memory minConfirmationsChange = minConfirmationsChanges[
                _minConfirmationsChangeId
            ];
        require(
            _minConfirmationsChangeId > 0 &&
                minConfirmationsChange.id == _minConfirmationsChangeId,
    ...
    function approveTransaction(
        uint256 _txId
    ) public onlyOwner activeTx(_txId) {
        require(
            !txApproved[_txId][msg.sender],
     ...
     */
    function proposeOwnersChange(
        address _ownerChanged,
        bool _addToList
    ) external onlyOwner maxProposalLimitNotExceeded {
        _addToList
     ...
     */
    function approveOwnersChange(
        uint256 _ownersChangeId
    ) public onlyOwner activeOwnersChange(_ownersChangeId) {
        require(
    ...
    function executeOwnersChange(
        ...
        if (ownersChange.addToList) {
            require(
                !isOwner[ownersChange.ownerChanged],
                "Governance: Owner not unique"
            );
     ...
     */
    function proposeMinConfirmationsChange(
        uint256 _newValue
    ) external onlyOwner maxProposalLimitNotExceeded {
        require(
     ...
     */
    function approveMinConfirmationsChange(
        uint256 _minConfirmationsChangeId
    ) public onlyOwner activeMinConfirmationsChange(_minConfirmationsChangeId) {
        require(
            !minConfirmationsChangeApproved[_minConfirmationsChangeId][
                msg.sender
            ],
     ...
     * It should have the minimum number of confirmations required in order to be executed.
     */
    function executeMinConfirmationsChange(
        uint256 _minConfirmationsChangeId
    ) public onlyOwner activeMinConfirmationsChange(_minConfirmationsChangeId) {
        MinConfirmationsChange
            memory minConfirmationsChange = minConfirmationsChanges[
        ...
        require(
            minConfirmationsChange.newValue > 0 &&
                minConfirmationsChange.newValue <= owners.length,
            "Governance: Invalid minimum confirmations"
        );
        ...
        emit ExecuteMinConfirmationsChange(
            msg.sender,
            _minConfirmationsChangeId
        );
    }
    ...
    function _cleanExpiredProposals() private {
        ...
        for (uint256 i = activeOwnersChangesIds.length; i > 0; --i) {
            OwnersChange memory ownersChange = ownersChanges[
                activeOwnersChangesIds[i - 1]
            ];
        ...
        for (uint256 i = activeMinConfirmationsChangesIds.length; i > 0; --i) {
            MinConfirmationsChange
                storage minConfirmationsChange = minConfirmationsChanges[
                    activeMinConfirmationsChangesIds[i - 1]
                ];
            if (minConfirmationsChange.deadline < block.timestamp) {
                activeMinConfirmationsChangesIds[
                    i - 1
                ] = activeMinConfirmationsChangesIds[
    ...
    function _cancelActiveProposals() private {
        ...
        for (uint256 i = activeMinConfirmationsChangesIds.length; i > 0; --i) {
            uint256 minConfirmationsChangeId = activeMinConfirmationsChangesIds[
                i - 1
            ];
            MinConfirmationsChange
                memory minConfirmationsChange = minConfirmationsChanges[
            ...
            if (
                ...
                minConfirmationsChanges[minConfirmationsChangeId]
                    .cancelled = true;
            }
            activeMinConfirmationsChangesIds.pop();
            --ownerActiveProposalsCount[minConfirmationsChange.proposer];
    ...
    function _removeFromActiveTransactionsIds(uint256 _txId) private {
        for (uint256 i; i < activeTransactionsIds.length; ++i) {
            if (activeTransactionsIds[i] == _txId) {
                activeTransactionsIds[i] = activeTransactionsIds[
                    activeTransactionsIds.length - 1
                ];
                activeTransactionsIds.pop();
