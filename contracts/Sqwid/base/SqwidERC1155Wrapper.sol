...
abstract contract SqwidERC1155Wrapper is ERC721Holder, ERC1155Holder {
    ...
    modifier wrappedExists(uint256 tokenId) {
        require(
            _wrappedTokens[tokenId].tokenId > 0,
            "Wrapper: Wrapped token not found"
        );
        _;
    }
    ...
    function getWrappedToken(
        uint256 tokenId
    ) public view wrappedExists(tokenId) returns (WrappedToken memory) {
