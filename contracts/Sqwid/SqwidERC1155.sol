    ...
    SqwidERC1155Wrapper
{
    ...
    function wrapERC721(
        ...
        require(
            IERC165(extNftContract).supportsInterface(
        ...
        IERC721(extNftContract).safeTransferFrom(
            msg.sender,
            address(this),
            extTokenId
        );
        ...
        if (tokenId > 0) {
            ...
            if (
                ...
                (royaltyRecipient, royaltyValue) = INftRoyalties(extNftContract)
                    .royaltyInfo(extTokenId, 10000);
            }
            string memory uri_ = IERC721Metadata(extNftContract).tokenURI(
                extTokenId
            );
            tokenId = mint(
                msg.sender,
            ...
            _wrappedTokens[tokenId] = WrappedToken(
                tokenId,
                true,
                extTokenId,
                extNftContract
    ...
    function wrapERC1155(
        ...
        require(
            IERC1155(extNftContract).balanceOf(msg.sender, extTokenId) >=
                amount,
        ...
        if (tokenId > 0) {
            ...
            uint256 royaltyValue;
            if (
                IERC1155(extNftContract).supportsInterface(
                    type(INftRoyalties).interfaceId
                )
            ) {
                (royaltyRecipient, royaltyValue) = INftRoyalties(extNftContract)
            ...
            string memory uri_ = IERC1155MetadataURI(extNftContract).uri(
                extTokenId
            );
            tokenId = mint(
                msg.sender,
    ...
    function _safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            require(
                fromBalance >= amount,
                "ERC1155: Insufficient token balance"
            );
            unchecked {
                _balances[id][from] = fromBalance - amount;
            }
    ...
    function _increaseSupply(
        uint256 tokenId,
        uint256 amount
    ) internal override wrappedExists(tokenId) {
        require(
